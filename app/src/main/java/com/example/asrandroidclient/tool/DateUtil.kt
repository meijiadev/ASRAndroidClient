...
object DateUtil {
    ...
    //转换时间格式
    fun convertOtherFormat(
        dateTime: String,
        curentFormatStr: String,
        targetFormatStr: String
    ): String {
    ...
    fun getDistanceDateByYear(distance: Int, formatStr: String, dateTime: String = ""): String {
        ...
        try {
            val date =
                if (TextUtils.isEmpty(dateTime)) df.parse(getCurrentDateTime(formatStr)) else df.parse(
                    dateTime
                )
    ...
    fun isInThePeriod(
        ...
        try {
            val key =
                if (TextUtils.isEmpty(dateTime)) df.parse(getCurrentDateTime(formatStr)).time else df.parse(
                    dateTime
                ).time
            val start = df.parse(startDate).time
            val end = df.parse(endDate).time
