package com.example.asrandroidclient.ivw
...
class IvwHelper(
    ...
    private fun initParams(
        ...
        if (ret != AbilityConstant.ABILITY_SUCCESS_CODE) {
            Logger.e("open ivw error code ===> $ret")
            mainThread {
        ...
        if (ret != AbilityConstant.ABILITY_SUCCESS_CODE) {
            Logger.w("open ivw specifyDataSet error code ===> $ret")
            mainThread {
        ...
        if (aiHandle?.code != AbilityConstant.ABILITY_SUCCESS_CODE) {
            ...
            Logger.w("open ivw start error code ===> $ret")
            mainThread {
    ...
    private fun writeData(audio: ByteArray) {
        ...
        synchronized(lockArray) {
            ...
            if (ret != AbilityConstant.ABILITY_SUCCESS_CODE) {
                //写入失败，暂停录音
                endAiHandle()
                Logger.w("writeData is error => $ret")
            } else {
    ...
    override fun onStartRecord() {
        Logger.i("onStartRecord===>")
        recordCallback?.onStartRecord()
    }

    override fun onPauseRecord() {
        Logger.i("onPauseRecord===>")
        recordCallback?.onPauseRecord()
    }

    override fun onResumeRecord() {
        Logger.i("onResumeRecord===>")
    ...
    override fun onStopRecord(output: File?) {
        Logger.i("onStopRecord===>${output?.absolutePath}")
    ...
    override fun onEvent(
        ...
        Logger.i("handleID:$handleID,event:$event")
    }
